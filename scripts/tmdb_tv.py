...
OUTPUT_FILE = "feed/tmdb_tv_rss.xml" # Cesta k výstupnímu souboru v adresáři feed/
PRAGUE_TZ = pytz.timezone('Europe/Prague') # Časová zóna pro lastBuildDate kanálu a pubDate (použijeme místní zónu)

# Pro TMDB není potřeba Build ID logika ani nastavení lokále (datum je v MM/DD/YYYY)
...
ET.SubElement(channel, 'title').text = 'TMDB Populární seriály (Vybrané služby)' # Titulek
ET.SubElement(channel, 'link').text = URL # Odkaz na zdrojovou stránku
ET.SubElement(channel, 'description').text = 'Aktuálně nejoblíbenější seriály na TheMovieDB.org dostupné na vybraných streamovacích službách' # Popis kanálu
ET.SubElement(channel, 'language').text = 'cs-cz' # Jazyk (protože používáme cs-CZ URL)
# Přidání odkazu na samotný RSS feed
atom_link = ET.SubElement(channel, "{http://www.w3.org/2005/Atom}link")
# !! DŮLEŽITÉ: Nahraď tuto URL skutečnou finální URL, kde bude feed hostován !!
atom_link.set("href", "https://raw.githubusercontent.com/scarzxx/kinobox-rss/refs/heads/main/feed/tmdb_tv_rss.xml") # Příklad URL - NEZAPOMEŇ ZMĚNIT
...
# --- Zpracování jednotlivých seriálů (itemů) ---
print("Zpracovávám seriály pro RSS feed...")
cards = soup.find_all('div', class_='card style_1')

if not cards:
    print("Nenalezeny žádné karty seriálů na stránce.", file=sys.stderr)
else:
    for card in cards:
        ...
        full_link = f"https://www.themoviedb.org{href}" if href else URL # Odkaz na detail seriálu, fallback na URL kanálu
        ...
        date_text = date_tag.get_text(strip=True) if date_tag else '' # Toto by mělo být datum prvního vysílání

        # Parsování data pro pubDate
        date_for_pubdate = None # Inicializujeme datum pro pubDate na None
        if date_text:
             try:
                 # Zkusíme parsovat datum z date_text (na stránce TMDB TV je obvykle MM/DD/YYYY)
                 date_for_pubdate = datetime.strptime(date_text, '%m/%d/%Y')
                  ...
                  if date_text.strip():
                      print(f" Varování: Nepodařilo se naparsovat datum '{date_text}' pro seriál '{title}' pro pubDate. PubDate vynechán.")
        ...
        if date_for_pubdate:
            # Přidáme časovou zónu (např. UTC, protože neznáme přesný čas prvního vysílání)
        ...
        # Tag <description> - S OBRÁZKEM, HODNOCENÍM A DATEM V HTML A CDATA
        ...
        # Přidat datum prvního vysílání, pokud existuje (date_text)
        if date_text.strip(): # Kontrolujeme, jestli text data není prázdný nebo jen mezery
             description_parts_html.append(f'<p>První vysílání: {date_text}</p>')
        ...
        if description_parts_html: # Tedy pokud jsme něco přidali (obrázek, hodnocení, datum)
        ...
        # Else: Pokud není k dispozici žádná doplňující data (obrázek, hodnocení, datum), tag <description> pro tento item nebude přidán.
...
try:
    ...
    # Zde pro jistotu nastavujeme encoding="utf-8" a xml_declaration=True.
    ...
    for item_elem in reparsed.getElementsByTagName('item'): # OPRAVENO: Používáme reparsed
        description_elements = item_elem.getElementsByTagName('description')
        if description_elements:
            desc_elem_dom = description_elements[0]
            # Zíkáme aktuální textový obsah z DOM elementu.
            ...
            cdata_section = reparsed.createCDATASection(current_text) # OPRAVENO: Používáme reparsed

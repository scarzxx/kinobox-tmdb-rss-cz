...
SubElement(channel, 'title').text = 'TMDB Seriály'
SubElement(channel, 'link').text = url
SubElement(channel, 'description').text = 'Oblíbené Seriály'

# Zpracování jednotlivých karet s daty
...
# Zajištění složky pro RSS feed
os.makedirs("feed", exist_ok=True)

# Uložení do souboru s formátováním
xml_str = tostring(rss, 'utf-8')
parsed_str = minidom.parseString(xml_str)
file_path = "feed/tmdb_tv_rss.xml"
with open(file_path, "w", encoding="utf-8") as f:
    f.write(parsed_str.toprettyxml(indent="  "))

# Git operace pro nahrání souboru přes SSH
repo_dir = '/home/scarzx/python/kinobox-tmdb-rss-cz'  # cesta do repozitáře
branch = 'main'  # nebo jiná větev

# Přechod do repozitáře
os.chdir(repo_dir)

# Git operace: přidání souboru, commit a push
subprocess.run(['git', 'add', file_path])
subprocess.run(['git', 'commit', '-m', 'Automatický upload RSS souboru'])
subprocess.run(['git', 'push', 'origin', branch])

print("Soubor byl úspěšně nahrán na GitHub pomocí SSH!")
